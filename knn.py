...
distancias = vizinhos(vetor,table)
distanciasOrdenadas = distancias[:]
distanciasOrdenadas.sort()

raio = int(input("Digite a quantidade de vizinhos (preferencialmente números ímpares):"))

for i in range(raio):
    posicao = distancias.index(distanciasOrdenadas[i])
print("O vizinho mais próximo é a {}.".format(table[posicao][-1]),)